...
const heroAsset = "/manus-storage/market-signal-global-network-user_1a55121c.png";
...
export default function Home() {
  ...
  const tradeLearning = trpc.market.tradeLearningPreview.useQuery();
  const tradeLearningHistory = trpc.market.tradeLearningHistory.useQuery(undefined, { enabled: isAuthenticated });
  ...
  const learningRecords = tradeLearningHistory.data?.records.length ? tradeLearningHistory.data.records : tradeLearning.data?.records ?? [];
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#070807] text-[#f4f5ec]">
      ...
      <main className="mx-auto max-w-[1580px] px-5 pb-16 pt-5 lg:px-8">
        ...
        <section className="mt-5 rounded-[1.35rem] border border-white/10 bg-[#101210] p-5 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><BrainCircuit className="h-4 w-4 text-[#d9fb63]" /><p className="mono text-[10px] uppercase tracking-[.18em] text-white/38">Trade learning ledger</p></div><h2 className="mt-2 text-2xl font-extrabold tracking-[-.05em]">Remember the evidence. Review the mistake.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-white/48">Every completed paper trade can retain its original evidence, P&L, error classification, and a lesson awaiting human review. The system does not rewrite strategy rules on its own.</p></div><div className="rounded-xl border border-[#d9fb63]/25 bg-[#d9fb63]/[.055] px-4 py-3"><p className="mono text-[9px] uppercase tracking-[.15em] text-[#d9fb63]">Calibration sample</p><p className="mt-1 text-xl font-extrabold">{tradeLearning.data?.calibration.sampleSize ?? 0} outcomes</p><p className="mt-1 text-xs text-white/44">{tradeLearning.data?.calibration.status === "insufficient_sample" ? "Insufficient sample — no accuracy claim." : "Segmented review required."}</p></div></div><div className="mt-5 grid gap-4 lg:grid-cols-2">{learningRecords.map(record => <article key={record.id} className="rounded-2xl border border-white/10 bg-black/20 p-4"><div className="flex items-start justify-between gap-4"><div><p className="mono text-[10px] text-white/38">{record.status.replace("_", " ")}</p><h3 className="mt-1 text-xl font-extrabold tracking-[-.05em]">{record.symbol} <span className="mono text-[10px] font-normal text-white/42">/ {record.side}</span></h3></div><span className={cn("mono text-sm font-bold", record.netPnl >= 0 ? "text-[#d9fb63]" : "text-[#ff8b8b]")}>{record.netPnl >= 0 ? "+" : ""}${record.netPnl.toFixed(2)}</span></div><div className="mt-4 grid gap-3 border-t border-white/10 pt-3 sm:grid-cols-2"><div><p className="mono text-[9px] uppercase tracking-[.14em] text-white/34">Source basis</p>{record.sources.map(source => <p key={source} className="mt-1 text-xs leading-5 text-white/58"><FileText className="mr-1 inline h-3 w-3 text-[#d9fb63]" />{source}</p>)}</div><div><p className="mono text-[9px] uppercase tracking-[.14em] text-white/34">Mistake taxonomy</p><p className="mt-1 text-xs font-bold text-white/76">{record.errorCategory.replace(/_/g, " ")}</p><p className="mt-2 text-xs leading-5 text-white/55">{record.lesson}</p></div></div></article>)}</div><p className="mt-4 flex items-start gap-2 border-t border-white/10 pt-4 text-xs leading-5 text-white/40"><ShieldCheck className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#d9fb63]" />{tradeLearningHistory.data?.notice ?? tradeLearning.data?.notice}</p></section>
